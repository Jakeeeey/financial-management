...
export async function GET(request: NextRequest) {
  ...
  if (!token && !authDisabled) {
    console.warn('[DailyCollectionReport] No token found in cookie or Authorization header');
  ...
  if (!base) {
    console.error('[DailyCollectionReport] SPRING_API_BASE_URL is not set');
  ...
  try {
    console.log('[DailyCollectionReport] Fetching:', targetUrl);
    ...
    if (!springRes.ok) {
      ...
      console.error('[DailyCollectionReport] Upstream error:', springRes.status, text.slice(0, 200));
    ...
    console.error('[DailyCollectionReport]', errorString);
