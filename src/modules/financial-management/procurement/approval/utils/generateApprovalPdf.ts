  ...
  const doc = await PdfEngine.generateWithFrame(
    ...
    (doc, startY, config) => {
      ...
      autoTable(doc, {
        ...
        styles: { fontSize, lineColor: [0, 0, 0], lineWidth: 0.3 },
        footStyles: { fillColor: [128, 128, 128], textColor: 255, fontSize, fontStyle: "bold" },
      ...
      doc.line(leftX, sigTop + 12, leftX + 55, sigTop + 12);
      doc.line(rightX - 55, sigTop + 12, rightX, sigTop + 12);

      const detailY = sigTop + 16;
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      const preparedDetail = options.preparedByName ?? "";
      if (preparedDetail) {
        doc.text(truncate(preparedDetail, 50), leftX, detailY, { baseline: "top" });
      }
      const approvedDetail = options.approvedByName ?? "";
      if (approvedDetail) {
        doc.text(truncate(approvedDetail, 50), rightX - 55, detailY, { baseline: "top" });
      }
    }
  );
