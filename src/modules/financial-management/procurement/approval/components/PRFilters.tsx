  ...
  const rightGroup = (
    <div className="flex flex-col lg:flex-row items-end gap-3 shrink-0 w-full">
      <div className="w-full lg:w-[200px] shrink-0">
        ...
        <Combobox items={supplierNameItems} value={supplierLabel ?? ""} open={supplierOpen} onOpenChange={setSupplierOpen}
          onValueChange={(name: string | null) => {
            if (name) { const id = supplierIdByName.current[name] ?? ""; onSupplierChange(id, name); }
            else { onSupplierChange("", null); setSupplierSearchText(""); }
          }}
        >
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col lg:flex-row lg:items-end lg:gap-3 lg:justify-between">
        ...
        <div className="hidden lg:flex items-end gap-3 shrink-0">{rightGroup}</div>
      </div>
