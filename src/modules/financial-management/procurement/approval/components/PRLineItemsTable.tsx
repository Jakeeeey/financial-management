...
function ItemTemplateCombobox({
  value, templateId, onSelect, disabled,
}: {
  value: string;
  ...
  function handleValueChange(selected: string | null) {
    ...
    if (parts[0] === "tmpl" && parts[1]) {
      ...
      if (cached) onSelect(cached);
      else onSelect({ id, name: parts.slice(2).join(":"), uom: null, base_price: null, description: null });
    }
  }
...
function VariantSelect({
  templateId, variantId, onChange, disabled,
}: {
  templateId: number | null;
  ...
  if (!templateId || variants.length === 0) return <span className="text-xs text-muted-foreground">&mdash;</span>;

  return (
    <Select value={variantId ? String(variantId) : ""}
      onValueChange={(v) => { const found = variants.find((x) => x.id === Number(v)); onChange(found ?? null); }}
      disabled={disabled}
    >
...
export function PRLineItemsTable({
  details, procurementId, readOnly, onDetailUpdated, onDetailDeleted, onDetailAdded,
  ...
  async function startEdit(d: ProcurementDetail) {
    if (editingId !== null && editingId !== d.id) { await handleSave(editingId); }
    setEditingId(d.id);
    setEditQty(d.qty);
  ...
  function handleCancel() { setEditingId(null); }

  async function handleDelete(detailId: number) {
    onDetailDeleted(detailId);
    setDeletingId(detailId);
    try { await deletePRDetail(detailId); toast.success("Line item removed"); }
  ...
  function handleTemplateSelect(id: number, tmpl: { id: number; name: string; uom: string | null; description: string | null; base_price: number | null } | null) {
    if (!tmpl) return;
    updateNewRow(id, { item_template_id: tmpl.id, item_variant_id: null, template_name: tmpl.name, variant_name: null, uom: tmpl.uom ?? "", unit_price: tmpl.base_price ?? 0 });
  }

  function handleVariantSelect(id: number, variant: { id: number; name: string; list_price: number | null } | null) {
    updateNewRow(id, { item_variant_id: variant?.id ?? null, unit_price: variant?.list_price ?? 0, variant_name: variant?.name ?? null });
  }
  ...
  async function handleSaveNewRow(id: number) {
    ...
    try {
      const created = await createPRDetail({
        procurement_id: procurementId, item_template_id: row.item_template_id,
        item_variant_id: row.item_variant_id, qty: row.qty, unit_price: row.unit_price, uom: row.uom || undefined,
      });
      setNewRows((prev) => prev.filter((r) => r.id !== id));
      onDetailAdded({ ...created, template_name: row.template_name, variant_name: row.variant_name });
  ...
  function handleDiscardNewRow(id: number) { setNewRows((prev) => prev.filter((r) => r.id !== id)); }

  const tempDetails = newRows.map((r) => ({
    id: r.id, procurement_id: procurementId, qty: r.qty, unit_price: r.unit_price,
    total_amount: r.qty * r.unit_price, uom: r.uom || null, item_variant_id: r.item_variant_id || null,
    item_template_id: r.item_template_id || null, date_added: null, link: null,
    created_at: null, updated_at: null, template_name: r.template_name ?? null, variant_name: r.variant_name ?? null,
    supplier: null,
    ...
    <div>
      {!readOnly && (
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          <Button variant="outline" size="sm" onClick={handleAddRow}><Plus className="h-4 w-4 mr-1" /> Add Line Item</Button>
      ...
      <div className="rounded-md border overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={readOnly ? 5 : 7} className="text-sm text-muted-foreground text-center py-8">{filterText ? "No matching line items" : "No line items"}</td></tr>
            )}
            {filtered.map((d) => {
              const isNew = d.id < 0;
                  ...
                  {!readOnly && (
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      {isNew ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => startEdit(d)}><span className="text-xs">Edit</span></Button>
