...
export default function PrintProcurementDialog({
  ...
  const handleDownloadFromPreview = async () => {
    ...
    link.download = `PROCUREMENT_${procurementNo}_${Date.now()}.pdf`;
    link.click();
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Print Procurement</DialogTitle>
          </DialogHeader>
          ...
          >
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium mt-1">{procurementNo}</p>
              </div>
