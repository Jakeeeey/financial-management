...
interface VATSupplierPieChartProps {
  data: VATCustomerEntry[]; // already sorted descending by value
}

export function VATSupplierPieChart({ data }: VATSupplierPieChartProps) {
  const total = data.reduce((s, d) => s + d.value, 0);

  // Split into top N + rest grouped as "Others"
  ...
  const chartData: VATCustomerEntry[] = [
    ...top,
    ...(othersTotal > 0
      ? [{ name: `Others (${rest.length})`, value: othersTotal, color: '#94a3b8' }]
      : []),
  ];
  ...
  return (
    <Card className="shadow-none">
      ...
      <CardContent className="pt-4">
        ...
        {/* Inline legend for top N + Others */}
          ...
          {chartData.map((entry, i) => (
            <div key={i} className="flex items-center gap-1.5 min-w-0">
              <span
                ...
                style={{
                  backgroundColor: entry.name.startsWith('Others')
                    ? '#94a3b8'
          ...
          <details className="group">
            ...
            <div className="max-h-52 overflow-y-auto divide-y divide-border/40">
              {data.map((entry, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between px-4 py-2 text-xs hover:bg-muted/20 transition-colors"
                >
