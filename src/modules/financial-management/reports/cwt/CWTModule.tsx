...
export default function CWTModule() {
  ...
  const filteredMetrics    = useMemo((): CWTMetrics => isFiltered ? deriveMetrics(filtered)        : metrics,    [filtered, isFiltered, metrics]);
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      ...
      head: [['Invoice No.', 'Customer', 'Gross Amount (PHP)', 'Taxable Amount (PHP)', 'CWT Amount (PHP)', 'Date']],
  ...
  return (
    <div className="p-8 bg-background text-foreground min-h-screen space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/* From date */}
        <div className="flex items-center gap-1.5 rounded-md border border-border bg-background px-3 h-9 overflow-hidden">
          <span className="text-xs font-medium text-muted-foreground shrink-0">From</span>
          <input
            type="date"
            value={dateFrom}
            onChange={(e) => { setDateFrom(e.target.value); setPage(1); }}
            className="min-w-0 w-[120px] border-0 bg-transparent text-xs focus:outline-none focus:ring-0 p-0"
          />
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="shadow-none">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">Total CWT Amount</CardTitle>
        ...
        <Card className="shadow-none">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground">Average CWT</CardTitle>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <CWTBarChart data={displayAggregated} />
        <CWTPieChart data={displayAggregated} pieTotal={pieTotal} />
      </div>

      {/* Table */}
      <CWTRecordsTable records={displayRecords} page={page} setPage={setPage} />
