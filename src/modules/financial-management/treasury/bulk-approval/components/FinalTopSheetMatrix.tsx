...
function buildEncoderTotals(data: FinalTopSheetResponse) {
  ...
  for (const coaRow of data.coa_rows) {
    for (const cell of coaRow.cells) {
      totals.set(cell.employee_id, (totals.get(cell.employee_id) ?? 0) + cell.amount);
    }
  }
...
function ActionButtonSet({
  ...
  return (
    <div className="flex items-center justify-end gap-1" aria-label={label}>
      <Button
        type="button"
        size="icon"
      ...
      <div className="shrink-0 border-b bg-white px-5 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-[10px] text-slate-400">
                Stage decisions using toggles, then submit the batch below.
              </p>
            </div>
            ...
            {stagedCount > 0 && (
              <div className="flex items-center gap-2 animate-in slide-in-from-left-2 duration-300">
                ...
                >
                  ...
                  Submit {stagedCount} Staged Decision{stagedCount !== 1 ? "s" : ""}
                </Button>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2">
            {!canAct ? (
              <div className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-1.5 text-amber-800" title={readOnlyReason}>
                <LockKeyhole className="h-3.5 w-3.5" />
                <span className="text-[9px] font-black uppercase tracking-widest text-nowrap">View Only</span>
              </div>
            ) : null}
            <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100">
              <span className="text-[9px] font-black uppercase tracking-widest text-emerald-800/60">Total</span>
              <span className="text-sm font-black text-emerald-700">{formatCurrency(data.grand_total)}</span>
            </div>
          </div>
        </div>
      </div>
              ...
              {data.coa_rows.map((coaRow) => (
                ...
                >
                  <div className="flex flex-col items-end">
                    <span className="block max-w-[70px] truncate text-[7px] font-black uppercase tracking-tight text-slate-500" title={coaRow.account_title}>
          ...
          <TableBody>
            {data.salesmen.length === 0 ? (
              ...
              data.salesmen.map((salesman) => {
                ...
                const encoderLabel = salesman.salesman_name || `Employee #${salesman.employee_id}`;

                return (
                  <TableRow key={salesman.employee_id} className="group transition-colors hover:bg-slate-50/50">
                    <TableCell className="sticky left-0 z-20 bg-white group-hover:bg-slate-50/80 backdrop-blur-sm px-3 py-1.5 border-r border-b shadow-[5px_0_15px_-10px_rgba(0,0,0,0.05)] transition-all">
                      <button
                        type="button"
                        className="flex items-center gap-1.5 w-full rounded-lg transition-all hover:bg-primary/10 active:scale-[0.98] px-1 py-0.5 group/auditee"
                      ...
                      >
                        ...
                        <div className="min-w-0 text-left">
                          <p className="truncate text-[10px] font-black text-slate-800 group-hover/auditee:text-primary transition-colors" title={encoderLabel}>
                            {encoderLabel}
                          </p>
                          <p className="text-[8px] text-primary/60 font-bold opacity-0 group-hover/auditee:opacity-100 transition-opacity leading-none">Click to inspect →</p>
                        </div>
                      </button>
                    </TableCell>
                      ...
                      return (
                        <TableCell 
                          ...
                          className={`p-0 border-r border-b last:border-r-0 align-top group/cell transition-colors ${
                            activeStatus === "Approved" ? "bg-emerald-50/50" : 
                            activeStatus === "With Concern" ? "bg-amber-50/50" :
                            activeStatus === "Rejected" ? "bg-rose-50/50" : ""
                          }`}
                        >
                          {cell ? (
                            <div className="flex h-full min-h-[70px] w-full flex-col items-end justify-between gap-2 px-2 py-2">
                              <div className="flex w-full flex-col items-end justify-start">
                                <span className={`text-[10px] font-black tabular-nums ${cell.amount > 0 ? "text-slate-800" : "text-slate-300"}`}>
                                  {formatCurrency(cell.amount)}
                                </span>
                                <div className="flex items-center gap-1">
                                  <span className="text-[8px] font-bold text-slate-400">{cell.count} line{cell.count !== 1 ? "s" : ""}</span>
                                  {cell.has_concern && (
                              ...
                              <ActionButtonSet
                                ...
                                onToggle={(status) => onToggleDecision(status, target)}
                              />
                            </div>
                          ) : (
                    ...
                    <TableCell className="sticky right-[100px] z-20 bg-slate-50 group-hover:bg-slate-100 backdrop-blur-sm px-2 py-1.5 text-right border-b transition-all">
                      <p className="text-[10px] font-black text-slate-900 tabular-nums">
                        {formatCurrency(rowTotal)}
                      </p>
                    </TableCell>

                    <TableCell className={`sticky right-0 z-20 backdrop-blur-sm px-3 py-1.5 border-b transition-all ${
                      stagedDecisions[`encoder:${salesman.employee_id}`] === "Approved" ? "bg-emerald-50" :
                      stagedDecisions[`encoder:${salesman.employee_id}`] === "With Concern" ? "bg-amber-50" :
                      stagedDecisions[`encoder:${salesman.employee_id}`] === "Rejected" ? "bg-rose-50" : "bg-white group-hover:bg-slate-50"
                    }`}>
                      <ActionButtonSet
                        disabled={submitting || !canAct || rowTotal <= 0}
                        label={encoderLabel}
                        activeStatus={stagedDecisions[`encoder:${salesman.employee_id}`] ?? null}
                        onToggle={(status) => 
            ...
            <TableRow className="sticky bottom-0 z-30 bg-white/95 backdrop-blur-xl">
              ...
              {data.coa_rows.map((coaRow) => (
                <TableCell key={coaRow.coa_id} className="bg-slate-900/95 px-2 py-2 text-right border-r border-white/5 last:border-r-0">
              ...
              <TableCell className="sticky right-0 z-40 bg-emerald-600 px-4 py-2 text-right">
                <span className="text-[9px] font-black uppercase text-white/60">
                  {isApprovedHistory ? "Finalized" : canAct ? "Ready" : "Waiting"}
                </span>
              </TableCell>
            </TableRow>
          </TableBody>
