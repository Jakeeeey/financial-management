...
async function resolveLinkedTopSheetData(params: {
  ...
  const headerFilter: Record<string, unknown> = {
    id: params.headerId ? { _eq: params.headerId } : { _in: linkedHeaderIds },
  };
...
export async function getExpenseHeaderGroups(params: {
  ...
  for (const approvalDivisionId of finalDivisionIds) {
    ...
    for (const header of linked.headers) {
      ...
      // Unique key per header submission
      const key = `${approvalDivisionId}|${headerId}|${periodFrom}|${periodTo}`;
      ...
      const creatorId = toNumericId(header.created_by);
      const creatorName = creatorId ? (creatorMap.get(creatorId) ?? `User #${creatorId}`) : "Unknown User";
...
export async function handleFinalTopSheetsGetResource(params: {
  ...
  if (params.resource === "final-topsheet") {
    ...
    const headerId = toNumber(params.searchParams.get("header_id"));
