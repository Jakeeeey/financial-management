...
import type { SupplierDebitMemo, Supplier, ChartOfAccount, CreateDebitMemoPayload, MemoFilters } from "../types";
...
export function useSupplierDebitMemo() {
  const [memos,     setMemos]     = useState<SupplierDebitMemo[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = extractList<SupplierDebitMemo>(json);
      setMemos(data);
