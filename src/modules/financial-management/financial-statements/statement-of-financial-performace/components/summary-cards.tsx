...
export function SummaryCards() {
  const { data, isLoading } = useFinancialPerformance();

  const summary = data?.summary;
  
  const totalSales = summary?.grossSales ?? 0;
  const cogs = summary?.costOfGoodsSold ?? 0;
  // Total expenses = Operating Expenses + Other Expense
  const totalExpenses = (summary?.operatingExpenses ?? 0) + (summary?.otherExpense ?? 0);
  const netOtherIncome = summary?.netOtherIncome ?? 0;
    ...
    <div className="flex flex-wrap gap-4 mb-6">
      ...
      <div className="flex-1 min-w-[200px] border border-border bg-card rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
        ...
        <p className="text-[11px] text-muted-foreground font-medium">
          Income: {formatAmount(summary?.otherIncome)} | Expense: {formatAmount(summary?.otherExpense)}
        </p>
      </div>
