...
import { styleSupplierBatchWorksheet } from "../../shared/supplier-batch/supplierBatchExcelStyles";
...
export async function exportProductPrintablesExcel(args: {
    ...
    if (supplierName) {
        ws.addRow(["Supplier", supplierName]);
    }
    if (filterSummary) {
    ...
    const headers = [
        ...IDENTITY_HEADERS,
        ...(includeListCost
            ? unitColumns.map((unit) => currentListCostHeader(unit.label))
            : []),
        ...sortedPriceTypes.flatMap((priceType) =>
            unitColumns.map((unit) => priceCurrentHeader(priceType, unit.label)),
        ),
    ];
