...
export function childVariantIdsForGroup(
    index: Map<number, number[]>,
    groupId: number,
    parentProductId: number,
): number[] {
    const variantIds = index.get(groupId) ?? [];
    return variantIds.filter((id) => id !== parentProductId);
}

export function buildVariantGroupIndex(
    groups: Array<{ group_id: number; variant_product_ids: number[] }>,
): Map<number, number[]> {
    const map = new Map<number, number[]>();
    for (const group of groups) {
        map.set(group.group_id, group.variant_product_ids);
    }
    return map;
}
