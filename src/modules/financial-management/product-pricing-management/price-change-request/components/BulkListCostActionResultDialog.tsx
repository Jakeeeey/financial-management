...
function snapshotForId(snapshots: ListCostSelectionSnapshot[], requestId: number) {
    return snapshots.find((item) => item.request_id === requestId);
}

export function BulkListCostActionResultDialog({ open, onOpenChange, result, snapshots }: Props) {
    const rows = React.useMemo(() => {
        ...
        return attemptedIds.map((requestId) => {
            ...
            return {
                requestId,
                recordLabel: snapshot?.record_label ?? `CCR-${requestId}`,
                productLabel: snapshot?.product_label ?? `CCR-${requestId}`,
                succeeded,
                statusLabel:
                        ...
                        <TableBody>
                            {rows.map((row) => (
                                <TableRow key={row.requestId}>
                                    ...
                                    <TableCell className="max-w-[220px] truncate" title={row.productLabel}>
                                        {row.productLabel}
                                    </TableCell>
                                    <TableCell>
                                        <Badge
